...
| 2026-08-23 | Issue #42: implemented tunnel-only `publish-1` administration — a WireGuard role, the contract rule that the SSH allowlist must be exactly the VPN subnet, and convergence ordering that proves the tunnel before the firewall can close the public path. The server key is generated on the host once and escrowed in the password manager, matching the existing rule for age keys and provider logins rather than requiring SOPS. Monthly cost unchanged at `$0`. |
