...
- [x] ~~Postiz vs Mixpost~~ → **Postiz**, self-hosted AGPL-3.0-or-later edition
      pinned at `v2.23.0`, founder-selected 2026-08-25. Later controlled drills
      found that HTTP-successful cancellation can leave its Temporal workflow
      `RUNNING`, and that rebuilding Elasticsearch without reindexing loses the
      exact Visibility lookup (`1` before, `0` after). These are explicit issue
      #17 (`WP-13`) implementation conditions: scheduler-verified kill switch
      plus retained Visibility state or a rehearsed reindex. They do not reopen
      the issue #16 decision/evidence gate. The complete packet is
...
| 2026-08-27 | PR #49 later review evidence completed the `DG-01` handoff without changing the Postiz choice: `DELETE /public/v1/posts/<id>` can remove the row while its Temporal workflow remains `RUNNING`, so issue #17 must verify cancellation/kill-switch behavior at scheduler level. The controlled restore also measured the exact Visibility query at `1` before Elasticsearch destruction and `0` after rebuild while the retained workflow stayed `RUNNING`; issue #17 must retain that state or rehearse a working reindex. The founder clarified that issue #16 is the complete decision/evidence plan and issue #17 owns implementation. Monthly cost unchanged at `$0`. |
| 2026-08-27 | Issue #17 (`WP-13`) added the inactive selected Postiz `v2.23.0` desired state: digest-pinned six-container Compose, R2-only media, retained Postiz PostgreSQL/Redis plus Temporal PostgreSQL/Elasticsearch Visibility, project/brand/provider-grant mapping enforcement, scheduler-level termination and globally serialized freeze controls, and an effective-config-verified outbound-blocked disposable dump/restore adapter. Activation remains blocked on live `publish-1` (#45), tunnel/Access/R2 (#14), backup foundation (#15), exact-host founder plan approval, and the seven-day/provider canaries. Monthly cost unchanged at `$0`. |
