...
| 2026-08-27 | Issue #17 (`WP-13`) added the inactive selected Postiz `v2.23.0` desired state: digest-pinned six-container Compose, R2-only media, retained Temporal PostgreSQL and Elasticsearch Visibility, project/brand/provider-grant mapping enforcement, scheduler-level termination and global freeze controls, and an outbound-blocked disposable dump/restore adapter. Activation remains blocked on live `publish-1` (#45), tunnel/Access/R2 (#14), backup foundation (#15), exact-host founder plan approval, and the seven-day/provider canaries. Monthly cost unchanged at `$0`. |
