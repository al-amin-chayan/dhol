...
from backend import BackendError, NoRedirect, S3, digest, require_ciphertext, session, snapshot
from control_plane import no_change_plan
from edge import documents, render, validate
from receipt import input_digest

ACCOUNT = "7512591000a1e57593bc784dad59bfc0"
ZONE = "2c295efab57307e714b212bcab2d7c42"
...
def run(command, directory=None, env=None, data=None, codes=(0,)):
    result = subprocess.run(command, cwd=directory, env=env, input=data,
                            capture_output=True, timeout=180)
    if result.returncode not in codes:
        raise OperationError(f"{command[0]} {command[1] if len(command) > 1 else ''} failed (exit {result.returncode}); output suppressed")
    return result
...
def secret_set():
    ...
    for key in ("/run/founder.age", "/run/break-glass.age"):
        recovered.append(yaml.safe_load(run(["sops", "--decrypt", str(path)],
                                           env={**os.environ, "SOPS_AGE_KEY_FILE": key}).stdout))
    ...
    if (set(document) != {"schema_version", "secret_set_id", "owner_project_id", "values"}
            or document.get("secret_set_id") != "cloudflare" or document.get("schema_version") != 1
...
def ensure_secret_set():
    if SECRET_PATH.is_file() or (EVIDENCE / "cloudflare.sops.yml").is_file():
        return secret_set()
    document = {"schema_version": 1, "secret_set_id": "cloudflare", "owner_project_id": "platform",
                "values": {"platform-opentofu-state-passphrase": secrets.token_urlsafe(48)}}
    cipher = run(["sops", "--encrypt", "--filename-override", "infra/secrets/cloudflare.sops.yml",
                  "--input-type", "yaml", "--output-type", "yaml", "/dev/stdin"],
                 directory=ROOT, data=yaml.safe_dump(document).encode()).stdout
    for key in ("/run/founder.age", "/run/break-glass.age"):
        recovered = run(["sops", "--decrypt", "--input-type", "yaml", "--output-type", "yaml", "/dev/stdin"],
                        env={**os.environ, "SOPS_AGE_KEY_FILE": key}, data=cipher).stdout
...
def bucket_session(inputs, bucket, prefixes):
    ...
    return session(ACCOUNT, verified["id"], digest(inputs["CLOUDFLARE_R2_BOOTSTRAP_TOKEN"].encode()), bucket, prefixes)


def backend_config(key):
    config = yaml.safe_load((PACKAGE / "bootstrap.yml").read_text())["backend"]
    return {"bucket": config["bucket"], "key": key, "region": "auto", "use_lockfile": True,
            ...
            "skip_s3_checksum": True, "use_path_style": True}


def environment(inputs, values, bucket, prefixes):
    return {**os.environ, **bucket_session(inputs, bucket, prefixes),
...
def initialize(directory, env, key):
    ...
    run(["tofu", "init", "-input=false", "-backend-config=" + str(config), "-lockfile=readonly"], directory, env)
...
def executable_provider(directory):
    ...
    path = directory / f".terraform/providers/registry.opentofu.org/cloudflare/cloudflare/5.24.0/linux_{architecture}/terraform-provider-cloudflare_v5.24.0"
    if path.is_file():
...
def prepare(directory):
    ...
    (directory / "core-ingress.json").write_text(json.dumps(render(*docs, "core-1")["ingress"]))
    return json.loads((PACKAGE / "adoption.json").read_text())


def provider_validate(inputs):
    with tempfile.TemporaryDirectory(prefix="dholbeat-provider-validation-") as temporary:
        directory = Path(temporary)
        prepare(directory)
        env = {**os.environ, "TF_VAR_state_passphrase": secrets.token_urlsafe(48),
               "TF_INPUT": "0", "CHECKPOINT_DISABLE": "1"}
        run(["tofu", "init", "-backend=false", "-lockfile=readonly", "-input=false"], directory, env)
        executable_provider(directory)
        result = json.loads(run(["tofu", "validate", "-json"], directory, env, codes=(0, 1)).stdout)
        if not result.get("valid"):
            for diagnostic in result.get("diagnostics", []):
                # Summaries identify provider schema errors without displaying source snippets.
                print(json.dumps({"severity": diagnostic.get("severity"), "summary": diagnostic.get("summary")}))
            raise OperationError("locked provider validation failed")
        evidence("validate", {"provider_version": "5.24.0", "opentofu_version": "1.12.5",
                              "provider_lock_sha256": digest((PACKAGE / ".terraform.lock.hcl").read_bytes())})
...
def plan(inputs, adopt=False):
    ...
    storage = S3(ACCOUNT, config["bucket"], {k: env[k] for k in ("AWS_ACCESS_KEY_ID", "AWS_SECRET_ACCESS_KEY", "AWS_SESSION_TOKEN")})
    ...
    with tempfile.TemporaryDirectory(prefix="dholbeat-cloudflare-plan-") as temporary:
        ...
        if adopt:
            imports = "\n".join('import {\n  to = ' + address + '\n  id = ' + json.dumps(identity["import_id"]) + '\n}'
                                for address, identity in sorted(expected.items()))
            (directory / "imports.tf").write_text(imports + "\n")
        binary = directory / "plan.encrypted"
        result = run(["tofu", "plan", "-json", "-input=false", "-detailed-exitcode", "-out=" + str(binary)], directory, env, codes=(0, 1, 2))
        if result.returncode == 1:
            for line in result.stdout.splitlines():
                ...
                if event.get("type") == "diagnostic":
                    ...
                    print(json.dumps({"severity": diagnostic.get("severity"), "summary": diagnostic.get("summary")}))
            raise OperationError("native provider plan failed; no resource changes applied")
        document = json.loads(run(["tofu", "show", "-json", str(binary)], directory, env).stdout)
        try:
            ...
            for change in document.get("resource_changes", []):
                ...
                if delta["actions"] != ["no-op"]:
                    ...
                    print(json.dumps({"resource": change["address"], "actions": delta["actions"],
                                      "changed_attributes": sorted(k for k in set(before) | set(after) if before.get(k) != after.get(k))}))
            raise OperationError("provider plan is not no-change; nothing was applied") from None
        ...
        if adopt:
            ...
            run(["tofu", "plan", "-input=false", "-detailed-exitcode", "-out=" + str(binary)], directory, env)
            document = json.loads(run(["tofu", "show", "-json", str(binary)], directory, env).stdout)
            no_change_plan(document, expected)
        receipt = {"schema_version": 1, "package": "cloudflare", "provider_mutations": 0,
                   "resource_count": len(expected), "provider_version": "5.24.0", "opentofu_version": "1.12.5",
                   "encrypted_plan_sha256": digest(binary.read_bytes()),
                   "inputs_sha256": source_digest, "imported": adopt, "observed_epoch": time.time(),
                   "backend_bucket": config["bucket"], "backend_key": config["key"]}
...
def take_snapshot(inputs, emit=True):
    config = yaml.safe_load((PACKAGE / "bootstrap.yml").read_text())["backend"]
    source = S3(ACCOUNT, config["bucket"], bucket_session(inputs, config["bucket"], ["cloudflare/"]))
    recovery = S3(ACCOUNT, config["recovery_bucket"], bucket_session(inputs, config["recovery_bucket"], ["snapshots/"]))
...
def bootstrap(inputs):
    ...
    if initial and (config.get("bucket_id") is not None or config.get("recovery_bucket_id") is not None):
        raise OperationError("initial bootstrap must not replace pinned roots")
    if {config["bucket"], config["recovery_bucket"]} != {"dholbeat-tfstate", "dholbeat-tfstate-recovery"}:
        raise OperationError("bootstrap root names differ from the reviewed Dholbeat boundary")
    bindings = {}
    for name in (config["bucket"], config["recovery_bucket"]):
        if name not in names:
            if not initial:
                raise OperationError("immutable backend root is missing; restore it through the documented bootstrap gate")
            api.request("POST", f"/accounts/{ACCOUNT}/r2/buckets", {"name": name, "locationHint": "apac", "storageClass": "Standard"})
        public = api.request("GET", f"/accounts/{ACCOUNT}/r2/buckets/{name}/domains/managed")
        expected_id = config["bucket_id"] if name == config["bucket"] else config["recovery_bucket_id"]
        bindings[name] = public.get("bucketId")
        if not bindings[name] or (not initial and bindings[name] != expected_id):
            raise OperationError("immutable backend bucket identity differs; refusing adoption")
        custom = api.request("GET", f"/accounts/{ACCOUNT}/r2/buckets/{name}/domains/custom")
        lifecycle = api.request("GET", f"/accounts/{ACCOUNT}/r2/buckets/{name}/lifecycle")
        rules = lifecycle.get("rules", [])
        safe_multipart = all(set(rule) <= {"id", "enabled", "conditions", "abortMultipartUploadsTransition"}
                             and "abortMultipartUploadsTransition" in rule for rule in rules)
        if public.get("enabled") or custom.get("domains") or not safe_multipart:
            raise OperationError("immutable backend root is public or expires state objects; refusing adoption")
    if initial:
        evidence("bootstrap-provision", {"account_id": ACCOUNT, "immutable_root_candidates": bindings,
                                         "public": False, "state_initialized": False})
        return
    ensure_secret_set()
    recovery = S3(ACCOUNT, config["recovery_bucket"], bucket_session(inputs, config["recovery_bucket"], ["recovery/", "snapshots/", "drills/"]))
    cipher = SECRET_PATH.read_bytes() if SECRET_PATH.is_file() else (EVIDENCE / "cloudflare.sops.yml").read_bytes()
    stored = recovery.request("GET", "recovery/cloudflare.sops.yml", missing_ok=True)
    if stored is None:
        recovery.request("PUT", "recovery/cloudflare.sops.yml", cipher, headers={"if-none-match": "*"})
    elif stored != cipher:
        raise OperationError("independent credential root differs or is absent; use the explicit recovery/rotation gate")
    if digest(recovery.request("GET", "recovery/cloudflare.sops.yml")) != digest(cipher):
        raise OperationError("independent encrypted credential recovery read-back failed")
    evidence("bootstrap", {"account_id": ACCOUNT, "bucket": config["bucket"],
                           "recovery_bucket": config["recovery_bucket"], "public": False,
                           "credential_ciphertext_sha256": digest(cipher), "local_state_authority": False})
...
def recovery_drill(inputs):
    ...
    storage = S3(ACCOUNT, config["bucket"], {k: env[k] for k in ("AWS_ACCESS_KEY_ID", "AWS_SECRET_ACCESS_KEY", "AWS_SESSION_TOKEN")})
    recovery = S3(ACCOUNT, config["recovery_bucket"], bucket_session(inputs, config["recovery_bucket"], ["drills/"]))
    if len(storage.list("drills/")) >= 20:
        raise OperationError("disposable drill inventory reached its bound; recover locks and clean known drills first")
    owned = {}
    try:
        ...
        for store, object_key in ((storage, key), (recovery, owned.get("snapshot_key", ""))):
            if object_key:
                store.request("DELETE", object_key, expected=(204,))

    evidence("recovery-drill", {"native_lock_contention": True, "wrong_unlock_rejected": True,
                                "clean_clone_no_change": True, "wrong_encryption_key_rejected": True,
                                "independent_ciphertext_recovered": True, "disposable_objects_removed": True})
...
def _recovery_drill(inputs, values, config, key, env, storage, recovery, owned):
    with tempfile.TemporaryDirectory(prefix="dholbeat-lock-drill-") as temporary:
        ...
        (directory / "drill.tf").write_text('resource "terraform_data" "proof" { input = "disposable-lock-recovery-proof" }\n')
        ...
        lock = {"ID": lock_id, "Operation": "OperationTypeApply", "Info": "Dholbeat disposable contention drill",
                "Who": "controller@disposable", "Version": "1.12.5",
                "Created": __import__("datetime").datetime.now(__import__("datetime").timezone.utc).isoformat(),
                "Path": config["bucket"] + "/" + key}
        storage.request("PUT", key + ".tflock", json.dumps(lock).encode(), headers={"if-none-match": "*"})
        try:
            contender = run(["tofu", "apply", "-auto-approve", "-input=false", "-lock-timeout=0s"], clone, env, codes=(1,))
            if b"Error acquiring the state lock" not in contender.stderr + contender.stdout:
                raise OperationError("native lock contention did not reject the second writer")
            run(["tofu", "force-unlock", "-force", "wrong-lock-id"], clone, env, codes=(1,))
        ...
        storage.request("PUT", key, recovery.request("GET", copied["snapshot_key"]), headers={"if-none-match": "*"})
        restored = json.loads(run(["tofu", "state", "pull"], clone, env).stdout)
        if restored["lineage"] != state["lineage"] or restored["resources"] != state["resources"]:
...
def credentials(path: Path) -> dict[str, str]:
    allowed = {"CLOUDFLARE_API_TOKEN", "CF_ACCOUNT_ID", "CLOUDFLARE_R2_BOOTSTRAP_TOKEN",
               "CLOUDFLARE_R2_PARENT_TOKEN", "R2_PARENT_ACCESS_KEY_ID", "R2_PARENT_SECRET_ACCESS_KEY"}
    allowed |= {"N8N_ACCESS_CLIENT_ID", "N8N_ACCESS_CLIENT_SECRET", "PUBLISHER_API_KEY", "PROBE_CORE_1_IP", "PROBE_PUBLISH_1_IP"}
    allowed |= {"FOUNDER_ACCESS_JWT_" + identifier.replace("-", "_").upper() for identifier in ("paperclip-admin", "n8n-admin", "publisher-admin-api")}
    result = {}
...
class Cloudflare:
    ...
    def request(self, method: str, path: str, body=None):
        ...
        try:
            ...
            raise OperationError(f"Cloudflare operation rejected (HTTP {error.code})") from None
...
def discover(inputs):
    for key in ("CLOUDFLARE_API_TOKEN", "CLOUDFLARE_R2_BOOTSTRAP_TOKEN", "CLOUDFLARE_R2_PARENT_TOKEN"):
        if not inputs.get(key):
            continue
        api = Cloudflare(inputs[key])
        for path in (f"/accounts/{ACCOUNT}/r2/buckets", f"/accounts/{ACCOUNT}/cfd_tunnel", f"/zones/{ZONE}"):
            try:
                api.request("GET", path)
                print(json.dumps({"credential_reference": key, "endpoint": path, "status": "read permitted"}))
            except OperationError as error:
                print(json.dumps({"credential_reference": key, "endpoint": path, "status": str(error)}))
        for path in (f"/accounts/{ACCOUNT}/tokens/permission_groups", "/user/tokens/permission_groups"):
            try:
                groups = api.request("GET", path)
                print(json.dumps({"credential_reference": key, "endpoint": path,
                                  "permission_groups": [{"id": g["id"], "name": g["name"]} for g in groups]}))
            except OperationError as error:
                print(json.dumps({"credential_reference": key, "endpoint": path, "status": str(error)}))
...
def verify(inputs):
    ...
    application_id = expected["cloudflare_zero_trust_access_application.team"]["resource_id"]
    policy_id = expected["cloudflare_zero_trust_access_policy.founder"]["resource_id"]
    application = api.request("GET", f"/accounts/{ACCOUNT}/access/apps/{application_id}")
    policy = api.request("GET", f"/accounts/{ACCOUNT}/access/policies/{policy_id}")
    verify_founder_policy(application, policy, "team.chayan.me")
    zone = api.request("GET", f"/zones/{ZONE}")
    registrar = api.request("GET", f"/accounts/{ACCOUNT}/registrar/domains/chayan.me")
    if set(zone["name_servers"]) != set(docs[2]["domains"][0]["expected_nameservers"]):
        raise OperationError("provider nameservers differ")
    if registrar.get("auto_renew") is not True or registrar.get("expires_at") != docs[3]["renewal_evidence"]["expires_on"]:
    ...
    for parent in docs[3]["delegation_evidence"]["independent_parent_servers"]:
        ...
        if nameservers(parent, "chayan.me") != expected_ns:
            raise OperationError("independent parent delegation differs")
    check("unauthenticated", request("team.chayan.me"))
    # This child imports existing resources only. It must report future security gates honestly.
    deferred = {route["id"]: route["status"] for route in docs[0]["routes"] if route["status"] != "adopted"}
    evidence("verify", {"existing_founder_access": True, "unauthenticated_denied": True,
                        "independent_parent_delegation": True, "registrar_renewal": True,
                        "promotion_probe_contracts": matrix(docs[0]), "deferred_routes": deferred,
                        "core_host_contacted": False, "provider_mutations": 0})


def validate_recovery_request(request, config):
    if (set(request) != {"schema_version", "snapshot_key", "sha256", "backend_bucket", "backend_key", "expected_primary_absent"}
            or request["schema_version"] != 1 or request["expected_primary_absent"] is not True
            or request["backend_bucket"] != config["bucket"] or request["backend_key"] != config["key"]
            or not re.fullmatch(r"snapshots/[0-9]{8}T[0-9]{12}Z-[a-f0-9]{64}\.tfstate", request["snapshot_key"])
            or not re.fullmatch(r"[a-f0-9]{64}", request["sha256"])):
        raise OperationError("recovery request does not bind the absent primary and reviewed snapshot")
...
def restore(inputs):
    ...
    values = secret_set()
    primary = S3(ACCOUNT, config["bucket"], bucket_session(inputs, config["bucket"], ["cloudflare/"]))
    recovery_env = environment(inputs, values, config["recovery_bucket"], ["snapshots/"])
    recovery = S3(ACCOUNT, config["recovery_bucket"], {k: recovery_env[k] for k in ("AWS_ACCESS_KEY_ID", "AWS_SECRET_ACCESS_KEY", "AWS_SESSION_TOKEN")})
    ...
    with tempfile.TemporaryDirectory(prefix="dholbeat-independent-recovery-") as temporary:
        directory = Path(temporary)
        expected = prepare(directory)
        coordinates = {**backend_config(request["snapshot_key"]), "bucket": config["recovery_bucket"]}
        (directory / "backend.json").write_text(json.dumps(coordinates))
        run(["tofu", "init", "-input=false", "-backend-config=" + str(directory / "backend.json"), "-lockfile=readonly"], directory, recovery_env)
        executable_provider(directory)
        state = json.loads(run(["tofu", "state", "pull"], directory, recovery_env).stdout)
        identities = {}
        ...
        if identities != {address: identity["resource_id"] for address, identity in expected.items()}:
    ...
    evidence("restore", {"provider_mutations": 0, "encrypted_state_sha256": digest(cipher),
                         "resource_count": len(expected), "independent_decryption_verified": True})
...
def clean_drills(inputs):
    ...
    if any(not re.fullmatch(r"drills/[a-f0-9]{32}\.tfstate", key) for key in keys):
        raise OperationError("drill inventory includes a lock or unknown object; nothing removed")
    eligible = []
    with tempfile.TemporaryDirectory(prefix="dholbeat-disposable-cleanup-") as temporary:
        versions = (PACKAGE / "versions.tf").read_text()
        versions = versions[:versions.index("  required_providers {")] + versions[versions.index('  backend "s3" {}'):]
        for key in keys:
            directory = Path(temporary) / key.split("/")[-1]
            directory.mkdir()
            (directory / "versions.tf").write_text(versions.replace('provider "cloudflare" {}', ''))
            raw = storage.request("GET", key)
            require_ciphertext(raw)
            initialize(directory, env, key)
            state = json.loads(run(["tofu", "state", "pull"], directory, env).stdout)
            resources = state.get("resources", [])
            if (len(resources) != 1 or resources[0].get("type") != "terraform_data" or resources[0].get("name") != "proof"
                    or resources[0]["instances"][0]["attributes"].get("input") != {"type": "string", "value": "disposable-lock-recovery-proof"}):
                raise OperationError("state is not the disposable drill; nothing removed")
            eligible.append((key, raw))
    for key, raw in eligible:
        if storage.request("GET", key + ".tflock", missing_ok=True) is not None or storage.request("GET", key) != raw:
            raise OperationError("drill is locked or changed; refusing cleanup")
        storage.request("DELETE", key, expected=(204,))
    evidence("clean-drills", {"disposable_objects_removed": len(eligible), "production_state_untouched": True})


def main():
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("operation", choices=["discover", "validate", "bootstrap", "adopt", "plan", "verify", "recovery-drill", "snapshot", "probe", "restore", "clean-drills"])
    ...
    if args.operation == "discover":
        ...
        complete = all(all(value in {"accepted", "denied"} for value in result.values()) for result in results.values())
        evidence("probe", {"provider_mutations": 0, "complete": complete, "results": results})


if __name__ == "__main__":
    try:
        main()
    except (OperationError, BackendError, OSError, ValueError, KeyError, subprocess.TimeoutExpired) as error:
        # Never include provider response bodies, subprocess output or values.
        print(str(error) if isinstance(error, (OperationError, BackendError)) else "control-plane operation failed safely")
