...
def name(data, offset):
    ...
    visited=set()
    while True:
        require(offset < len(data) and offset not in visited and len(visited) < 128, 'invalid DNS name')
        visited.add(offset)
        size=data[offset]
        if size & 0xc0 == 0xc0:
...
def parse(data, transaction, domain):
    ...
    identity,flags,questions,answers,authority,additional=struct.unpack('!6H',data[:12])
    require(identity==transaction and flags & 0x8000 and not flags & 0x020f and not flags & 0x0100
            and questions==1 and answers==0 and 0<authority<=32 and additional<=32, 'invalid parent DNS response')
    question,offset=name(data,12)
    require(question==domain and offset+4<=len(data) and data[offset:offset+4]==b'\x00\x02\x00\x01','DNS question differs')
    offset+=4
    result=set()
    for index in range(authority+additional):
        owner,offset=name(data,offset)
        require(offset+10<=len(data), 'truncated DNS record')
        kind,record_class,ttl,size=struct.unpack('!HHIH',data[offset:offset+10]);offset+=10
...
def nameservers(parent, domain):
    ...
    question=b''.join(bytes([len(label)])+label.encode('ascii') for label in domain.split('.'))+b'\x00\x00\x02\x00\x01'
