...
def session(account: str, access_key: str, secret_key: str, bucket: str,
            prefixes: list[str], now: int | None = None) -> dict[str, str]:
    ...
    payload = {"bucket": bucket, "scope": "object-read-write", "sub": account,
               "iss": access_key, "aud": endpoint, "iat": issued, "exp": issued + 3600,
               "paths": {"prefixPaths": prefixes, "objectPaths": []}}
    def encode(value):
        return base64.urlsafe_b64encode(json.dumps(value, separators=(",", ":")).encode()).rstrip(b"=")
    content = encode({"alg": "HS256", "typ": "JWT"}) + b"." + encode(payload)
    signature = base64.urlsafe_b64encode(hmac.digest(secret_key.encode(), content, "sha256")).rstrip(b"=")
    jwt = content + b"." + signature
    return {"AWS_ACCESS_KEY_ID": access_key, "AWS_SECRET_ACCESS_KEY": digest(jwt),
...
class S3:
    ...
    def request(self, method: str, key: str = "", body: bytes = b"", query=None, headers=None,
                expected=(200,), now: datetime | None = None, missing_ok=False) -> bytes:
        timestamp = (now or datetime.now(timezone.utc)).strftime("%Y%m%dT%H%M%SZ")
        day = timestamp[:8]
        path = "/" + urllib.parse.quote(self.bucket + "/" + key, safe="/~")
        pairs = sorted((urllib.parse.quote(str(k), safe="~"), urllib.parse.quote(str(v), safe="~"))
                       for k, v in (query or {}).items())
        query_string = "&".join(k + "=" + v for k, v in pairs)
        signed = {"host": self.host, "x-amz-date": timestamp,
                  "x-amz-content-sha256": digest(body),
                  "x-amz-security-token": self.credentials["AWS_SESSION_TOKEN"], **(headers or {})}
        signed = {k.lower(): " ".join(v.split()) for k, v in signed.items()}
        names = ";".join(sorted(signed))
        canonical_headers = "".join(f"{k}:{signed[k]}\n" for k in sorted(signed))
        canonical = "\n".join((method, path, query_string, canonical_headers, names, digest(body)))
        ...
        signed["authorization"] = ("AWS4-HMAC-SHA256 Credential=" + self.credentials["AWS_ACCESS_KEY_ID"]
                                   + "/" + scope + ", SignedHeaders=" + names + ", Signature=" + signature)
        request = urllib.request.Request("https://" + self.host + path + ("?" + query_string if query_string else ""),
    ...
    def list(self, prefix: str) -> list[str]:
        document = ET.fromstring(self.request("GET", query={"list-type": 2, "prefix": prefix, "max-keys": 1000}))
...
def require_ciphertext(data: bytes) -> None:
    ...
    try:
        ...
        raise BackendError("state is not a client-encrypted OpenTofu envelope") from None
    if not isinstance(envelope, dict) or "encrypted_data" not in envelope or "encryption_version" not in envelope:
...
def snapshot(source: S3, recovery: S3, key: str, prefix: str = "snapshots/") -> dict[str, str | int]:
    data = source.request("GET", key)
    require_ciphertext(data)
    sha = digest(data)
    target = prefix + datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%S%fZ") + "-" + sha + ".tfstate"
    recovery.request("PUT", target, data, headers={"if-none-match": "*"}, expected=(200,))
    try:
        ...
        import re
        if any(not re.fullmatch(re.escape(prefix) + r"[0-9]{8}T[0-9]{12}Z-[a-f0-9]{64}\.tfstate", k) for k in objects):
        ...
        # Remove only this successfully uploaded candidate; previous snapshots remain intact.
        recovery.request("DELETE", target, expected=(204,))
        raise
    return {"snapshot_key": target, "sha256": sha, "bytes": len(data), "retention_count": MAX_SNAPSHOTS}
