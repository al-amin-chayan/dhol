...
def test_current_and_future_ingress_derive_from_one_manifest():
    ...
    assert core["ingress"] == [{"hostname": "team.chayan.me", "service": "http://localhost:3100"}, {"service": "http_status:404"}]
    ...
    assert publisher["ingress"] == [{"hostname": "publish.chayan.me", "service": "http://127.0.0.1:5000"}, {"service": "http_status:404"}]
...
@pytest.mark.parametrize("mutation", [
    ...
    lambda d: d[0]["tunnels"][1].update(credential_ref=d[0]["tunnels"][0]["credential_ref"]),
    ...
    lambda d: d[3]["delegation_evidence"].update(expected_nameservers=["wrong.test"]),
...
@pytest.mark.parametrize("path", ["/", "/webhook/", "/webhook/../admin", "/webhook/x?admin=1",
                                  "/webhook/%2e%2e/admin", "/webhook/x%2fadmin", "//webhook/x", "/webhook-test/x"])
def test_webhook_proxy_rejects_wrong_or_ambiguous_path(path):
    assert webhook_proxy.authorize("POST", path, "secret", "secret") == 404


@pytest.mark.parametrize("method", ["GET", "HEAD", "PUT", "PATCH", "DELETE", "OPTIONS", "TRACE"])
def test_webhook_proxy_only_accepts_post(method):
    assert webhook_proxy.authorize(method, "/webhook/approval", "secret", "secret") == 405


def test_webhook_proxy_checks_application_token():
    assert webhook_proxy.authorize("POST", "/webhook/approval", "valid", "valid") == 200
    assert webhook_proxy.authorize("POST", "/webhook/approval", "invalid", "valid") == 401
...
@pytest.mark.parametrize('mutation', ['shared-bucket-key','missing-backup','broad-service-token','wrong-machine-path'])
def test_complete_separate_bucket_and_machine_credentials(mutation):
    import copy
    docs=copy.deepcopy(edge.documents())
    manifest=docs[0]
    if mutation=='shared-bucket-key': manifest['buckets'][0]['credential_refs']=manifest['buckets'][1]['credential_refs']
    elif mutation=='missing-backup': manifest['buckets'].pop(0)
    elif mutation=='broad-service-token': manifest['routes'][3]['machine_policy']['allow_any_service_token']=True
    else: manifest['routes'][3]['machine_policy']['application_path']='/*'
    with pytest.raises(ValueError): edge.validate(*docs)
