...
@pytest.mark.parametrize(
    "kind,response",
    [
        ("founder", Response(200)),
        ("service-token", Response(200)),
        ("method", Response(405)),
        ("path", Response(404)),
        ("webhook-token", Response(401)),
    ],
)
...
@pytest.mark.parametrize(
    "kind", ["founder", "service-token", "method", "path", "webhook-token"]
)
def test_denied_positive_or_exposed_machine_is_rejected(kind):
    response = (
        Response(302, LOGIN) if kind in {"founder", "service-token"} else Response(200)
    )
    with pytest.raises(ContractError):
        check(kind, response)
...
def test_all_declared_routes_have_full_probe_matrix():
    ...
    assert {
        ...
        "direct-ip",
        "alternate-dns",
    } <= set(cases["paperclip-admin"])
    assert {"method", "path", "webhook-token"} <= set(cases["approval-webhooks"])
    assert {"service-token", "wrong-service-token", "path"} <= set(
        cases["publisher-admin-api"]
    )
...
def test_missing_or_bypass_access_policy_fails():
    ...
    app = {
        "domain": "team.chayan.me",
        "type": "self_hosted",
        "policies": [{"id": "founder"}],
        "options_preflight_bypass": False,
    }
    verify_founder_policy(app, policy, "team.chayan.me", "mail@chayan.me")
    for changed in [{**app, "policies": []}, {**app, "options_preflight_bypass": True}]:
        with pytest.raises(ContractError):
            verify_founder_policy(changed, policy, "team.chayan.me", "mail@chayan.me")
    with pytest.raises(ContractError):
