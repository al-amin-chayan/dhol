...
def relay():
    ...
    Handler.window_count = 0
    Handler.window_start = time.monotonic()
    ...
    def call(method="POST", path="/webhook/disposable", body=b"{}", headers=None):
        ...
        conn.request(
            ...
            body=body,
            headers={
                "X-Telegram-Bot-Api-Secret-Token": Proxy.secret,
                **(headers or {}),
            },
        )
...
def test_authorized_json_forwarded_without_caller_credentials(relay):
    ...
    assert (
        not {"Authorization", "Cookie", "X-Telegram-Bot-Api-Secret-Token"}
        & headers.keys()
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("GET", "/webhook/test", {}, 405),
        ("TRACE", "/webhook/test", {}, 405),
        ("POST", "/", {}, 404),
        ("POST", "/webhook/../admin", {}, 404),
        ("POST", "/webhook/%2e%2e/admin", {}, 404),
        ("POST", "/webhook/test?query=1", {}, 404),
    ],
)
...
@pytest.mark.parametrize(
    "body,status",
    [(b"not-json", 400), (b"[]", 400), (b"", 413), (b"x" * (BODY_LIMIT + 1), 413)],
)
