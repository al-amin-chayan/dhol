...
@pytest.fixture
def cli(tmp_path):
    repo=tmp_path/'repo';scripts=repo/'scripts';(scripts/'lib').mkdir(parents=True)
    for path in ['scripts/cloudflare','scripts/lib/common.sh']:
        shutil.copyfile(ROOT/path,repo/path)
    (scripts/'controller').write_text('#!/bin/sh\nprintf "image_id=sha256:test-only-locked-image\\n"\n')
    (scripts/'controller').chmod(0o755)
    binary=tmp_path/'bin';binary.mkdir();capture=tmp_path/'args.json'
    docker=binary/'docker';docker.write_text('#!/usr/bin/env python3\nimport json,os,sys\nopen(os.environ["TEST_DOCKER_ARGS"],"w").write(json.dumps(sys.argv[1:]))\n');docker.chmod(0o755)
    inputs=[]
    for filename in ['credentials.env','founder.age','break-glass.age']:
        path=tmp_path/filename;path.write_text('test-only-private-input');path.chmod(0o600);inputs.append(path)
    env={**os.environ,'PATH':str(binary)+':'+os.environ['PATH'],'TEST_DOCKER_ARGS':str(capture),'CI':'','GITHUB_ACTIONS':''}
    def invoke(operation='plan',paths=None,extra=None,environment=None):
        paths=paths or inputs
        shell = "function " + shlex.quote(str(scripts/'controller')) + "() { printf 'image_id=sha256:test-only-locked-image\\n'; }; "
        ...
        args=['bash','-c',shell,'test-only',operation,'--credentials',str(paths[0]),'--founder-key',str(paths[1]),'--break-glass-key',str(paths[2]),'--confirm','cloudflare-control-plane']+(extra or [])
        return subprocess.run(args,env={**env,**(environment or {})},capture_output=True,text=True)
...
@pytest.mark.parametrize('kind',['public-mode','symlink','repo-input','github-profile'])
def test_unsafe_inputs_fail_before_controller(cli,kind):
    invoke,inputs,capture,repo=cli
    paths=list(inputs)
    if kind=='public-mode': paths[0].chmod(0o644)
    elif kind=='symlink':
        path=inputs[0].parent/'alias';path.symlink_to(inputs[0]);paths[0]=path
    elif kind=='repo-input':
        path=repo/'secret';path.write_text('test-only');path.chmod(0o600);paths[0]=path
    else:
        path=inputs[0].parent/'github-agent-apps'/'claude.env';path.parent.mkdir();path.write_text('test-only-not-an-identity');path.chmod(0o600);paths[0]=path
