...
def encode(domain):
    return (
        b"".join(bytes([len(label)]) + label.encode() for label in domain.split("."))
        + b"\0"
    )
...
def test_independent_parent_referral():
    assert parse(packet(), 42, "chayan.me") == {
...
@pytest.mark.parametrize(
    "field,value",
    [(0, 43), (1, 0x8100), (1, 0x8200), (1, 0x8003), (2, 0), (3, 1), (4, 0)],
)
def test_invalid_header_rejected(field, value):
    original = packet()
    header = list(struct.unpack("!6H", original[:12]))
    header[field] = value
    with pytest.raises(ContractError):
        parse(struct.pack("!6H", *header) + original[12:], 42, "chayan.me")
...
@pytest.mark.parametrize(
    ...
    [
        ...
        packet()[:-1],
        packet() + b"junk",
        packet()[:12] + b"\xc0\x0c" + packet()[14:],
    ],
)
def test_truncated_or_cyclic_response_rejected(data):
    with pytest.raises(ContractError):
        parse(data, 42, "chayan.me")
