...
- diagnosis of Paperclip's failed/nested backup amplification and a verified/encrypted R2 recovery redesign that leaves the application unchanged;
...
- `git diff --check` passed before live-audit revision commit `9a522a7`.
...
- Adversarially review the Paperclip backup diagnosis: nested hourly dumps, failed `tar`, absent checksums, retention behavior, R2 encryption, and restore-before-delete sequence.
