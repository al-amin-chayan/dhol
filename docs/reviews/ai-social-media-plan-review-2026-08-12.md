...
| **Claude API** | **Keep, with model routing** | Budget $4–8/month for two brands | Use Haiku 4.5 for extraction, classification, caption variants, and simple rewrites. Use Sonnet 5 for weekly synthesis and final risk/quality review only. Batch processing gets a 50% token discount. Haiku 4.5 is $1/$5 per million input/output tokens. Sonnet 5's $2/$10 introductory rate ends 2026-08-31; use its durable $3/$15 standard rate in budgets after that date. Web search is $10 per 1,000 searches plus tokens. [Anthropic pricing](https://platform.claude.com/docs/en/about-claude/pricing), [Sonnet 5 pricing transition](https://platform.claude.com/docs/en/about-claude/models/whats-new-sonnet-5#pricing) |
...
For a durable monthly budget, use Sonnet 5's standard price that takes effect after the August 2026 introductory period:

- Haiku: `(0.8 × $1) + (0.15 × $5) = $1.55`
- Sonnet: `(0.2 × $3) + (0.04 × $15) = $1.20`
- 100 web searches: `$1.00`, plus the tokens returned by search
- **Illustrative subtotal from September 2026: $3.75**, before retries and unexpectedly large fetched pages

Through 2026-08-31, Sonnet 5's introductory $2/$10 rate makes the Sonnet line $0.80 and the subtotal $3.35. The proposed $4–8 LLM cap therefore holds on either rate. Batch jobs can lower token charges, but the budget should retain the cushion rather than assume every call receives the discount.
